...
private object SwipeKernel {
    ...
    private var currentConf: FundamentalConfigs? = null
     ...
     */
    private fun updateKernel(
        conf: FundamentalConfigs,
        sampleRate: Float,
    ): SwipeKernelData? {
        runCatching { conf.validate() }.onFailure {
            Log.e("FundamentalConfigs is invalid: $it")
            return null
        }

        // Calculate the candidate fundamental frequency.
        val centerSemitone = conf.centerSemitone
        val startSemitone = centerSemitone - conf.radiusSemitone
        val semitoneStep = 1f / conf.semitoneSampleNum.toFloat()
        val candidateLength = floor((conf.radiusSemitone * 2) / semitoneStep + 1).toInt()
        ...
        val kernels = candidateFrequency.map { fundamentalFreq ->
            ...
            harmonics.map { idx ->
                ...
                for (i in startIndex until oneThirdIndex) {
                    ...
                    kernel[i] += 0.5f * cos(2.0f * PI * freq / fundamentalFreq)
                }
                // Draw the second part of the kernel.
                for (i in oneThirdIndex until twoThirdIndex) {
                    val freq = frequencyXAxis[i]
                    kernel[i] += cos(2.0f * PI * freq / fundamentalFreq)
                }
 ...
 * @param sampleRate The sample rate of the wave.
 */
fun WavData.toFundamentalSwipePrime(
    conf: FundamentalConfigs,
    sampleRate: Float,
): PitchData {
    ...
    return PitchData(fundamental, corr)
}
 ...
 */
private fun WavData.fastFourierTransformSwipe(
    windowSize: Int,
    sampleRate: Float,
    ...
    val window = HanningWindow(windowSize).window
    ...
    val absoluteSpectrogram: Array<DoubleArray> = Array(frames.size) { i ->
        ...
        fft.transform()
        val magnitude = fft.getMagnitude()
